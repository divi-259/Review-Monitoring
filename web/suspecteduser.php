...
<br>
  ...
  <center><h1><font size="6px">SUSPECTED USERS</font></h1>
  <font size="4px" style="font-family:comic sans ms">
	
	<?php
	include ('connect.php');
	$q="SELECT uid,pid,review FROM reviews WHERE suspected=1";
   ...
   <?php 
		...
		{ 
			$uid=$row['uid'];
			$q1="SELECT * FROM user1 WHERE id='$uid'";
			...
			$username=$row1['email'];
			$name=$fname." ".$lname;
			$q2="SELECT * FROM reviews WHERE Suspected=1 AND uid='$uid'";
